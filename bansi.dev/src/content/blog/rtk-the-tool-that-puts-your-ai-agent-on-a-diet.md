...
RTK stands for **Rust Token Killer**. It's a single Rust binary with zero dependencies and under 10ms of overhead, sitting between your AI agent and the terminal. A proxy.
...
RTK is **not** a general-purpose text compressor. It doesn't summarize arbitrary text. It doesn't use an LLM to compress LLM input (that would be beautifully absurd). It's a **command-aware filter** that knows what `git diff` output looks like, what matters in it, and what doesn't.
